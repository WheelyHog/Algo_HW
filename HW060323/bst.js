...
class BinarySearchTree {
    ...
    inorder(handlerFunction) {
        this.inorderIntenal(this.root, handlerFunction)
    }

    inorderIntenal(current, handlerFunction) {
        if (current == null) return
        this.inorderIntenal(current.left, handlerFunction)
        handlerFunction(current)
        this.inorderIntenal(current.right, handlerFunction)
    }


    preorder(handlerFunction) {
        this.preorderIntenal(this.root, handlerFunction)
    }

    preorderIntenal(current, handlerFunction) {
        if (current == null) return
        handlerFunction(current)
        this.preorderIntenal(current.left, handlerFunction)
        this.preorderIntenal(current.right, handlerFunction)
    }


    postorder(handlerFunction) {
        this.postorderIntenal(this.root, handlerFunction)
    }

    postorderIntenal(current, handlerFunction) {
        if (current == null) return
        this.postorderIntenal(current.left, handlerFunction)
        this.postorderIntenal(current.right, handlerFunction)
        handlerFunction(current)
    }
